...
source.complete = function(_, request, callback)
  ...
  if client.opts.types ~= nil then
      if note.title and string.len(note.title) > 0 then
          for typename, type in pairs(client.opts.types) do
              new_notes_opts[#new_notes_opts + 1] = { label = search, note = note, template = type.template, type = typename }
          end
      end
  end
  ...
  for _, new_note_opts in ipairs(new_notes_opts) do
    ...
    if ref_type == completion.RefType.Wiki then
      ...
      if new_note_opts.type and new_note_opts.template then
          label = string.format("[[%s]] (create %s)", new_note_opts.label, new_note_opts.type)
      end
